  ...
  function handleFile(file: File) {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        // silent — still works without description
      } finally {
        setAnalyzing(false)
      }
    }
            ...
            >
              {imageUrl ? (
                <>
                  ...
                  <button onClick={e => { e.stopPropagation(); setImageUrl(null); setImageBase64(null); setImageDesc('') }}
                    className="absolute top-3 right-3 bg-black/60 rounded-full p-1.5 hover:bg-black/80 z-10">
                    <X className="w-4 h-4 text-white" />
                  </button>
                  {analyzing && (
                    <div className="absolute bottom-3 left-3 flex items-center gap-2 bg-black/70 rounded-lg px-3 py-1.5 z-10">
                      <RefreshCw className="w-3.5 h-3.5 text-pink-400 animate-spin" />
                      <span className="text-xs text-pink-300">Bild wird analysiert...</span>
                    </div>
                  )}
                  {imageDesc && !analyzing && (
                    <div className="absolute bottom-3 left-3 right-3 bg-black/70 rounded-lg px-3 py-1.5 z-10">
